...
public abstract partial class DomainCommandHandler<TCommand> : IDomainCommandHandler<TCommand>
{
    ...
    /// The logger.
    /// </summary>
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0052:Remove unread private members", Justification = "Used by code generation")]
    ...
    public async Task<ExecuteCommandResult> DoAsync(object command, Metadata metadata, IDomainAggregate? aggregate, CancellationToken cancellationToken)
    {
        ...
        LogExecutingCommand(metadata.Message.Name, metadata.DomainGlobalId, metadata.Message.Id, metadata.Context.CorrelationId, metadata.Context.UserId);
        ExecuteCommandResult result = await DoAsync(ToCommand(command), metadata, aggregate, cancellationToken).ConfigureAwait(false);
        if (result.Failed)
        {
            LogCommandFailed(metadata.Message.Name, metadata.DomainGlobalId, metadata.Message.Id, metadata.Context.CorrelationId, metadata.Context.UserId);
        }
        else
        {
            LogCommandExecutedSucceeded(metadata.Message.Name, metadata.DomainGlobalId, metadata.Message.Id, metadata.Context.CorrelationId, metadata.Context.UserId);
        }
    ...
    /// <exception cref="ArgumentException">Invalid command type.</exception>
    ...
    [LoggerMessage(
        EventId = 3,
        Level = LogLevel.Error,
        Message = "Aggregate is null for command {CommandName} on aggregate {DomainGlobalId}. MessageId='{MessageId}'; CorrelationId='{CorrelationId}'; UserId='{UserId}'.")]
    ...
    [LoggerMessage(
        ...
        Message = "Command {CommandName} on aggregate {DomainGlobalId} succeeded. MessageId='{MessageId}'; CorrelationId='{CorrelationId}'; UserId='{UserId}'.")]
    ...
    [LoggerMessage(
        ...
        Message = "Command {CommandName} on aggregate {DomainGlobalId} failed. MessageId='{MessageId}'; CorrelationId='{CorrelationId}'; UserId='{UserId}'.")]
    ...
    [LoggerMessage(
        EventId = 2,
        Level = LogLevel.Information,
        Message = "Executing command {CommandName} on aggregate {DomainGlobalId}. MessageId='{MessageId}'; CorrelationId='{CorrelationId}'; UserId='{UserId}'.")]
